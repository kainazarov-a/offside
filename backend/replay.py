...
async def run_replay(engine, fid, speed=30.0, status_cb=None):
    ...
    async with httpx.AsyncClient(timeout=30) as cl:
        ...
        if fx:
            ...
            if not c:
                say(f"fixture {fid} not found in snapshot or history sweep")
                return
        ...
        # история счёта
        say("downloading score history")
        r = await _vget(cl, cfg, f"/scores/historical/{fid}")
        if r.status_code != 200:
            say(f"score history: HTTP {r.status_code} (outside the 6h..2w window?)")
            return
        try:
            scores = _vjson(r)
            ...
            return
        say(f"score events: {len(scores)}")

        # история одсов: 44 окна по 5 мин — ПАРАЛЛЕЛЬНО (было последовательно ~20-30с,
        # стало ~3-5с; именно эта пауза выглядела для зрителя как «лаг»)
        say("downloading odds history (44 windows in parallel)")
        odds = []
        ...
        async def _win(k):
            ...
            async with sem:
                try:
                    r = await _vget(cl, cfg,
                                    f"/odds/updates/{day}/{hour}/{interval}")
                    if r.status_code == 200:
                        return [d for d in _vjson(r)
                                if str(d.get("FixtureId")) == str(fid)]
                except Exception:
                    pass
            return []

        for chunk in await asyncio.gather(*[_win(k) for k in range(44)]):
            odds += chunk
        say(f"odds updates: {len(odds)}")
        ...
        if not feed:
            say("empty history")
            return
        say(f"replaying x{int(speed)}: {m['home']} vs {m['away']}")
        prev = feed[0][1]
        try:
