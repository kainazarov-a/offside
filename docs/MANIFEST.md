...
- Р2-финал ✓ (последний деплой перед фризом): понятность повсюду — строки карточек Live
  подписаны ИМЕНАМИ КОМАНД (вместо home/away; draw остаётся), тултипы на строках/чипе,
  легенда с расшифровкой толщин/баров, человеческие хинты на Live/Agent/Lag/PnL,
  чип «YOUR REPLAY ×30» + «runs only in this browser tab»; пустые списки Replay/Verify
  сами переспрашиваются каждые 15с (холодный пул больше не требует F5); обрезка сырых
  стек-трейсов в бейдже Verify (160 симв); гард гонки двойного старта реплея;
  кеш-бастер v=4. СЕРВЕРНЫЙ ФИКС: Node 18→22 (NodeSource) — лечит ERR_REQUIRE_ESM,
  on-chain view на Verify снова живой (был сломан на проде — найдено по скрину
  собственника). Раскрывающиеся карточки/полоса турнира — осознанно в роадмап:
  риск крупной фичи на последнем деплое > ценность, а боль «непонятно» закрыта
  подписями и хинтами без единого лишнего клика
- ФРИЗ после этого деплоя до 29.07: код не трогаем, только смотрим логи
